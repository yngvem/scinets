...
if __name__ == "__main__":
    ...
    architecture = [
        ...
        },
        {
            "layer": "ResnetConv2D",
            "scope": "resnet2",
            "layer_params": {"out_size": 1, "k_size": 3, "strides": 2},
            "normalization": {"operator": "batch_normalization"},
            "activation": {"operator": "relu"},
        },
        {
    ...
    network = NeuralNet(
        x, architecture, verbose=True, name=name, is_training=dataset.is_training
    )
    network.set_loss(
        true_out=dataset.target, loss_function="sigmoid_cross_entropy_with_logits"
    )
    trainer = NetworkTrainer(network, epoch_size=len(dataset.train_data_reader))

    log_dicts = [
        {"log_name": "Log loss", "log_var": "loss", "log_type": "log_scalar"},
    ...
    with tf.Session() as sess:
        ...
        for i in trange(1000):
            ...
            val_summaries = sess.run(
                logger.val_summary_op, feed_dict={is_training: False}
            )
