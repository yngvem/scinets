...
if __name__ == "__main__":
    ...
    architecture = [
        {
            ...
            "regularizer": {
                ...
                "arguments": {"amount": 0.5, "name": "weight_decay"},
            },
        },
        {
            "layer": "conv2d",
    ...
    network.set_loss(
        true_labels=true_y, loss_function="softmax_cross_entropy_with_logits"
    )
    trainer = NetworkTrainer(network, mnist)
    log_dict = {
        "loss": [
            {"log_name": "Loss", "log_type": "scalar"},
            {"log_name": "Log loss", "log_type": "log_scalar"},
        ],
        "input": [{"log_name": "Images", "log_type": "image"}],
    }

    logger = TensorboardLogger(network, log_dict)

    correct_prediction = tf.equal(
        tf.argmax(network.out, 1), tf.argmax(network.true_labels, 1)
    )
    ...
    with tf.Session() as sess:
        ...
        val_accuracy, = sess.run(
            [network.accuracy],
            feed_dict={_x: batch[0], true_y: batch[1], network.is_training: False},
        )
        print("step %d, val accuracy %g" % (0, val_accuracy))

        for i in range(100):
            summaries, steps = trainer.train(
                sess, 10, additional_ops=[logger.train_summary_op]
            )
