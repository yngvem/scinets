...
def _generate_activation(activation, generate_string=False):
    ...
    ---------
    activation : str
        What initializer to use. Acceptable values are the keys of the `activation_functions`
        dictionary or None.
    generate_string : bool
    ...
    if activation in activation_functions:
        ...
        act_func = activation_functions[activation]
    else:
        raise ValueError('The activation function must be a key in the `activation_functions`'
                         ' dictionary.')
...
def _generate_regularizer(regularizer, generate_string=False):
    ...
    ---------
    regularizer : str
        What regularizer to use. Acceptable values are the keys of the `regularizers`
        dictionary or None.
    generate_string : bool
    ...
    if regularizer is None:
        ...
        reg_func = None
    elif regularizer in activation_functions:
        reg_str = str(regularizer)
        reg_func = regularizers[regularizer]
    else:
        raise ValueError('The regularizer must be a key in the `regularizers` dictionary or None.')
...
def fc_layer(x, out_size, use_bias=True, initializer='he', std=0.1, regularizer=None,
    ...
    regularizer : str
        What regularizer to use on the weights. Must be None or a key in the `regularizers`
        dictionary.
    batch_norm : bool
    ...
    activation : str
        What activation function to use. Must be key in the `activation_functions` dictionary.
    scope : str
...
def resnet_conv_2d(x, out_size, k_size=3, use_bias=True, initializer='he', std=0.1, regularizer=None,
    ...
    strides : int or array_like(length=2)
        ...
        strides.
    activation : str
        What activation function to use at the end of the layer. Must be a key in the
        `activation_functions` dictionary.
    scope : str
        The scope of this layer (two layers can't share scope).
    verbose : bool
        Wether intermediate steps should be printed in console.
